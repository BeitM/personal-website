...
export function AwardCard({ award }: { award: Award }) {
  return (
    <article className="card award-card">
      ...
      <div><div className="entry-type">AWARD // {award.category}</div><div className="card-topline"><span>{award.organization}</span><span>{award.year}</span></div><h3>{award.name}</h3><p>{award.context}</p>
